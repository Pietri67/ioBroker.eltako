import { isObject, isControllerUiUpgradeSupported, encrypt, decrypt, appNameLowerCase, getRootDir, execAsync, } from '@iobroker/js-controller-common-db/tools';
import { SUPPORTED_FEATURES } from '../../lib/adapter/constants.js';
 ...
 */
export function isMessageboxSupported(instanceCommon) {
 ...
 */
export function encryptArray(options) {
 ...
 * @param options keys, object and secret information
 */
export function decryptArray(options) {
    const { secret, obj, keys } = options;
    for (const attr of keys) {
        const val = obj[attr];
        if (typeof val === 'string') {
 ...
 */
export function getAdapterScopedPackageIdentifier(options) {
 ...
 */
export async function listInstalledNodeModules(namespace) {
    const packJson = (await fs.readJson(path.join(getRootDir(), 'package.json')));
    const dependencies = [];
 ...
 */
export async function requestModuleNameByUrl(url) {
