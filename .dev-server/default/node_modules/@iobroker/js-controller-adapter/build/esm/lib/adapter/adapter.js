...
import { DEFAULT_SECRET, ALIAS_STARTS_WITH, SYSTEM_ADMIN_USER, SYSTEM_ADMIN_GROUP, ERROR_PERMISSION, ACCESS_EVERY_READ, ACCESS_EVERY_WRITE, ACCESS_GROUP_WRITE, ACCESS_GROUP_READ, ACCESS_USER_WRITE, ACCESS_USER_READ, NO_PROTECT_ADAPTERS, STATE_QUALITY, } from '../../lib/adapter/constants.js';
...
export class AdapterClass extends EventEmitter {
    ...
    constructor(options) {
        ...
        const instance = parseInt(this._options.compactInstance !== undefined
            ? this._options.compactInstance
            : this._options.instance !== undefined
                ? this._options.instance
                : this._config.instance || 0, 10);
    ...
    terminate(reason, exitCode) {
        ...
        const isNotCritical = _exitCode === EXIT_CODES.ADAPTER_REQUESTED_TERMINATION ||
    ...
    setObject(id, obj, options, callback) {
        ...
        Validator.assertOptionalCallback(callback, 'callback');
        return this._setObject({ id, obj: obj, options, callback });
    }
        ...
        if (options.obj.type) {
            ...
            if (!options.obj.common) {
                ...
                if (options.obj.common.role && this._defaultObjs[options.obj.common.role]) {
                    options.obj.common = extend(true, {}, this._defaultObjs[options.obj.common.role], options.obj.common);
                }
                ...
                if ('custom' in options.obj.common &&
    ...
    extendObject(id, obj, options, callback) {
        ...
        return this._extendObject({ id, obj: obj, options, callback });
    }
    ...
    async _extendObject(options) {
        ...
        options.obj.user = options.obj.user || (options.options ? options.options.user : '') || SYSTEM_ADMIN_USER;
        options.obj.ts = options.obj.ts || Date.now();
        if ((options.obj.type && options.obj.type === 'state') ||
    ...
    setForeignObject(id, obj, options, callback) {
        ...
        return this._setForeignObject({ id, obj: obj, options, callback });
    }
    ...
    getEnums(_enumList, options, callback) {
        ...
        return this._getEnums({ _enumList: _enumList, options, callback });
    }
    ...
    async _setObjectNotExists(options) {
        ...
        try {
            ...
            return tools.maybeCallbackWithError(options.callback, `Could not check object existence of ${options.id}: ${e.message}`);
        }
        if (objExists === false) {
            if (!options.obj.from) {
    ...
    formatDate(dateObj, isDuration, _format) {
        ...
        return this._formatDate({ dateObj: dateObj, isDuration, _format });
    }
    ...
    setState(id, state, ack, options, callback) {
        ...
        return this._setState({ id, state: state, ack, options, callback });
    }
        ...
        if (fixedId.startsWith(ALIAS_STARTS_WITH)) {
            ...
            if (obj?.common?.alias?.id) {
                ...
                try {
                    ...
                    return tools.maybeCallbackWithError(callback, `Error validating alias id of ${fixedId}: ${e.message}`);
                }
                // check the rights
                let targetObj;
                try {
                ...
                this.outputCount++;
                try {
                    const res = await this.#states.setState(aliasId, tools.formatAliasValue({
    ...
    setStateChanged(id, state, ack, options, callback) {
        ...
        return this._setStateChanged({ id, state: state, ack, options, callback });
    }
        ...
        if (options?.user && options.user !== SYSTEM_ADMIN_USER) {
            ...
            if (id.startsWith(ALIAS_STARTS_WITH)) {
                ...
                if (obj?.common?.alias?.id) {
                    ...
                    this.outputCount++;
                    this.#states.setState(aliasId, tools.formatAliasValue({
                        sourceCommon: obj?.common,
            ...
            if (id.startsWith(ALIAS_STARTS_WITH)) {
                ...
                if (obj?.common?.alias?.id) {
                    ...
                    this.outputCount++;
                    this.#states.setState(targetId, tools.formatAliasValue({
                        sourceCommon: obj.common,
                        targetCommon: targetObj?.common,
                        state,
                        logger: this._logger,
        ...
        if (id.startsWith(ALIAS_STARTS_WITH)) {
            if (obj?.common?.alias?.id) {
                ...
                if (aliasId) {
                    ...
                    return tools.maybeCallbackWithError(callback, null, tools.formatAliasValue({
    ...
    async _subscribeForeignStates(_options) {
        ...
        if (Array.isArray(pattern)) {
            ...
            for (const aliasPattern of pattern) {
                if ((aliasPattern.startsWith(ALIAS_STARTS_WITH) || aliasPattern.includes('*')) &&
                ...
                for (const license of obj.native.licenses) {
                    try {
                        ...
                        if (decoded.name &&
                            ...
                            if (decoded.name.startsWith(`iobroker.${adapterName || this.name}`) &&
    ...
    async _prepareInitAdapter() {
        ...
        else if (!this._config.forceIfDisabled &&
