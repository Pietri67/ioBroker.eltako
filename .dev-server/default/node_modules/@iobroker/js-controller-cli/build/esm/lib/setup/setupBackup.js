...
export class BackupRestore {
    ...
    tmpDir = path.normalize(path.join(controllerDir, 'tmp'));
    bkpDir = path.normalize(path.join(controllerDir, 'backups'));
    objects;
    states;
    processExit;
    restartController;
    ...
    BACKUP_POSTFIX = `_backup${tools.appNameLowerCase}`;
    constructor(options) {
        options = options || {};
        if (!options.states) {
    ...
    async createBackup(name, noConfig = false) {
        ...
        // read iobroker.json
        try {
            config = (await fs.readJSON(tools.getConfigFileName()));
    ...
    async _restoreJsonlBackup(options) {
        ...
        if (await fs.pathExists(path.join(backupBaseDir, 'config.json'))) {
            config = (await fs.readJSON(path.join(backupBaseDir, 'config.json')));
    ...
    async _restoreAfterStop(options) {
        ...
        if (force) {
            ...
            try {
                await execAsync(`"${process.execPath}" "${path.join(controllerDir, `${tools.appName.toLowerCase()}.js`)}" setup`);
            }
            catch (e) {
    ...
    async restoreBackup(options) {
        ...
        if (!(await fs.pathExists(path.join(backupBasePath, 'backup.json'))) &&
            !(await fs.pathExists(path.join(backupBasePath, 'objects.jsonl')))) {
            console.error(`host.${this.hostname} Cannot find extracted file "${path.join(backupBasePath, 'backup.json')}" or "${path.join(backupBasePath, 'objects.jsonl')}"`);
            return { exitCode: EXIT_CODES.CANNOT_EXTRACT_FROM_ZIP, objects: this.objects, states: this.states };
        }
