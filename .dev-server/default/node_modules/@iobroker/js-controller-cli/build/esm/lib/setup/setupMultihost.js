    ...
    getConfig() {
        ...
        try {
            if (fs.existsSync(this.configName)) {
                ...
                config = fs.readJsonSync(path.join(tools.getControllerDir(), 'conf', `${tools.appName.toLowerCase()}-dist.json`));
            }
        }
        catch {
            config = fs.readJsonSync(path.join(tools.getControllerDir(), 'conf', `${tools.appName.toLowerCase()}-dist.json`));
    ...
    async showMHState(config, changed) {
        if (config.multihostService.enabled) {
            ...
            if (hasLocalStatesServer) {
                ...
                !warningShown &&
                    console.log(`Please check the binding of the configured ${config.states.type} server to allow remote connections.`);
            }
        }
        if (!changed) {
        ...
        mhClient.connect(ip, pass, async (err, oObjects, oStates, ipHost) => {
            if (err) {
                ...
                if (hasLocalObjectsServer || hasLocalStatesServer) {
                    callback(new Error(`IP Address of the remote host is ${tools.getLocalAddress()}. Connections from this host will not be accepted. Please change the configuration of this host to accept remote connections.`));
                }
                else {
