...
export class Users {
                ...
                if (!err) {
                    this.objects.getObjectList({ startkey: 'system.group.', endkey: 'system.group.\u9999' }, (err, groups) => {
                        ...
                        for (let i = 0; i < groups.rows.length; i++) {
                            ...
                            // find all groups
                            if (groups.rows[i].value.common.members &&
     ...
     * @param callback
     */
    addUserPrompt(username, groupName, password, callback) {
        if (!username) {
     ...
     * @param callback
     */
    async setUserPassword(username, password, callback) {
        if (!username) {
        ...
        if (!username && !password) {
            ...
            prompt.get(schema, (err, result) => {
                this.checkPassword(result.username, result.password, (err, res) => {
                    if (err || !res) {
                        return tools.maybeCallbackWithError(callback, `Password for user "${result.username}" does not match${err ? `: ${err.message}` : ''}`);
                    }
            ...
            prompt.get(schema, (err, result) => {
                this.checkPassword(username, result.password, (err, res) => {
                    if (err || !res) {
                        return tools.maybeCallbackWithError(callback, `Password for user "${username}" does not match${err ? `: ${err.message}` : ''}`);
                    }
                    return tools.maybeCallbackWithError(callback, null);
                });
            });
