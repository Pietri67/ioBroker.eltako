...
import { statesDbHasServer, isLocalStatesDbServer, isLocalObjectsDbServer, objectsDbHasServer, performObjectsInterview, performStatesInterview, } from '@iobroker/js-controller-common';
...
export class Setup {
    ...
    async informAboutPlugins(systemConfig) {
        ...
        for (const plugin of Object.keys(plugins)) {
            ...
            let enabledState;
            try {
                enabledState = await this.states.getStateAsync(`system.host.${tools.getHostName()}.plugins.${plugin}.enabled`);
            }
            catch {
                // ignore
            }
...
Please DO NOT copy files manually into ioBroker storage directories!`);
    ...
    async migrateObjects(newConfig, oldConfig) {
        ...
        if (oldConfig &&
            ...
            if ((oldConfig.states.type === newConfig.states.type ||
            ...
            if (allowMigration) {
                ...
                if (newConfig.objects.type !== oldConfig.objects.type &&
    ...
    async setupCustom() {
        ...
        if (Array.isArray(originalConfig.objects.host)) {
            console.log(`  - Sentinel-Master-Name: ${originalConfig.objects.sentinelName
        ...
        console.log('');
        let otype = rl.question(`Type of objects DB [(j)sonl, (f)ile, (r)edis, ...], default [${currentObjectsType}]: `, {
        ...
        const defaultObjectsHost = otype === originalConfig.objects.type ? originalConfig.objects.host : tools.getLocalAddress();
        ...
        const defaultObjectsPort = otype === originalConfig.objects.type && oHost === originalConfig.objects.host
        ...
        let stype = rl.question(`Type of states DB [(j)sonl, (f)file, (r)edis, ...], default [${defaultStatesType}]: `, {
        ...
        let defaultStatesHost = stype === originalConfig.states.type ? originalConfig.states.host : oHost || tools.getLocalAddress();
        if (stype === otype) {
        ...
        let defaultStatesPort = stype === originalConfig.states.type && sHost === originalConfig.states.host
        ...
        if (hasLocalStatesServer || hasLocalObjectsServer) {
            ...
            hname = rl.question(`Host name of this machine [${originalConfig && originalConfig.system
        ...
        if (fs.existsSync(path.join(CONTROLLER_DIR, '..', '..', 'node_modules'))) {
            try {
                if (fs.existsSync(path.join(CONTROLLER_DIR, 'reinstall.js'))) {
                    fs.writeFileSync(path.join(CONTROLLER_DIR, '..', '..', 'reinstall.js'), fs.readFileSync(path.join(CONTROLLER_DIR, 'reinstall.js')));
                }
            }
            catch (e) {
...
require('${path.normalize(`${thisDir}/..`)}/setup').execute();`;
                try {
                    if (fs.existsSync(path.join(CONTROLLER_DIR, 'killall.sh'))) {
                        fs.writeFileSync(path.join(CONTROLLER_DIR, '..', '..', 'killall.sh'), fs.readFileSync(path.join(CONTROLLER_DIR, 'killall.sh')), { mode: 492 /* 0754 */ });
                    }
                    if (fs.existsSync(path.join(CONTROLLER_DIR, 'reinstall.sh'))) {
                        fs.writeFileSync(path.join(CONTROLLER_DIR, '..', '..', 'reinstall.sh'), fs.readFileSync(path.join(CONTROLLER_DIR, 'reinstall.sh')), { mode: 492 /* 0754 */ });
                    }
                    if (!fs.existsSync(path.join(CONTROLLER_DIR, '..', '..', `${tools.appName.substring(0, 3)}`))) {
                        fs.writeFileSync(path.join(CONTROLLER_DIR, '..', '..', `${tools.appName.substring(0, 3)}`), startFile, {
                            mode: 492 /* 0754 */,
                        });
                    }
