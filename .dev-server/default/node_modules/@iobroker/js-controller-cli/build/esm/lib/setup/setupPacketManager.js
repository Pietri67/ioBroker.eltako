...
var LOG_LEVELS;
(function (LOG_LEVELS) {
    LOG_LEVELS[LOG_LEVELS["silly"] = 5] = "silly";
...
export class PacketManager {
    ...
    async _isDpkgAvailable() {
        try {
            ...
            // non zero exit code, however lets check if ok
            if ((err.stdout && err.stdout.includes('dpkg --help')) ||
    ...
    async listUpgradeablePackages() {
        ...
        const { stdout } = await execAsync(`${(this.sudo ? 'sudo ' : '') + this.manager} ${this.COMMANDS.listUpgradeable[this.manager]}`);
        const res = Buffer.isBuffer(stdout) ? stdout.toString('utf-8') : stdout;
        if (!res) {
            return [];
        }
