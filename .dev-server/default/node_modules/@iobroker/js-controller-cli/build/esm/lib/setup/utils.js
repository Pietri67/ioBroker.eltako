...
export async function getRepository(options) {
    ...
    if (!anyFound) {
        let message;
        if (repoArr.length) {
            message = `ERROR: No repositories defined matching "${repoArr.join(' | ')}". Please use one of ${Object.keys(systemRepos.native.repositories)
                .map(repo => `"${repo}"`)
                .join(', ')}.`;
        }
        else {
 ...
 */
export async function isVersionIgnored(options) {
 ...
 * @param options name and target version of the adapter
 */
export async function ignoreVersion(options) {
    const { adapterName, version, objects } = options;
 ...
 */
export async function recognizeVersion(options) {
