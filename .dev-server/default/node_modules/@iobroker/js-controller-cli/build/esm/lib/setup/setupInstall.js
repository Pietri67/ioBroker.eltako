...
export class Install {
    ...
    async _enumerateAdapterChannels(knownObjIDs, adapter, instance) {
        ...
        try {
            ...
            if (doc.rows.length) {
                ...
                knownObjIDs.push(...newObjs);
                if (newObjs.length > 0) {
                    console.log(`host.${hostname} Counted ${newObjs.length} channels of ${adapter}${instance !== undefined ? `.${instance}` : ''}`);
                }
            }
        }
        catch (err) {
    ...
    async _enumerateAdapterStateObjects(knownObjIDs, adapter, instance) {
        const adapterRegex = new RegExp(`^${adapter}${instance !== undefined ? `\\.${instance}` : ''}\\.`);
        const sysAdapterRegex = new RegExp(`^system\\.adapter\\.${adapter}${instance !== undefined ? `\\.${instance}` : ''}\\.`);
        try {
            ...
            if (doc.rows.length) {
                ...
                knownObjIDs.push(...newObjs);
                if (newObjs.length > 0) {
                    console.log(`host.${hostname} Counted ${newObjs.length} states of ${adapter}${instance !== undefined ? `.${instance}` : ''}`);
                }
            }
            ...
            if (doc.rows.length) {
                ...
                knownObjIDs.push(...newObjs);
                if (newObjs.length > 0) {
                    console.log(`host.${hostname} Counted ${newObjs.length} states of system.adapter.${adapter}${instance !== undefined ? `.${instance}` : ''}`);
                }
            }
        }
        catch (err) {
    ...
    async _enumerateAdapterDocs(knownObjIDs, adapter, instance) {
        ...
        try {
            ...
            if (doc.rows.length) {
                ...
                knownObjIDs.push(...newObjs);
                if (newObjs.length > 0) {
                    console.log(`host.${hostname} Counted ${newObjs.length} objects of ${adapter}${instance !== undefined ? `.${instance}` : ''}`);
                }
            }
        }
        catch (err) {
    ...
    async _removeInstancesInstalledNodeModules(adapter, instance) {
        ...
        const regex = new RegExp(`^@${tools.appNameLowerCase}-${adapter}.${instance !== undefined ? instance : '\\d+'}\\/.*`, 'g');
