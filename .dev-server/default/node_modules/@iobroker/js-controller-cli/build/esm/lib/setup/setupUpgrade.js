...
export class Upgrade {
            ...
            for (const dName in allDeps) {
                if (dName === 'js-controller') {
                    ...
                    for (const instance of gInstances) {
                        const instanceVersion = instance.value.common.version;
                        try {
                            if (!semver.satisfies(instanceVersion, globalDeps[dName], {
                                includePrerelease: true,
                            })) {
    ...
    async upgradeAdapter(repoUrlOrObject, adapter, forceDowngrade, autoConfirm, upgradeAll) {
        ...
        const finishUpgrade = async (name, ioPack) => {
            if (!ioPack) {
                ...
                try {
                    ...
                    ioPack = fs.readJSONSync(path.join(adapterDir, 'io-package.json'));
                }
                catch {
                    console.error(`Cannot find io-package.json in ${adapterDir}`);
                    return this.processExit(EXIT_CODES.MISSING_ADAPTER_FILES);
        ...
        const showUpgradeDialog = (installedVersion, targetVersion, adapterName) => {
            ...
            do {
                if (isUpgrade || isDowngrade) {
                    ...
                    answer = rl.question(`Would you like to ${isUpgrade ? 'upgrade' : 'downgrade'} ${adapter} from @${installedVersion} to @${version || repoAdapter.version} now? [(y)es, (n)o]: `, {
                        defaultInput: 'n',
                    });
                }
                else {
                    answer = rl.question(`Would you like to reinstall version ${version || repoAdapter.version} of ${adapter} now? [(y)es, (n)o]: `, {
                        defaultInput: 'n',
                    });
                }
        ...
        if (repoAdapter.version) {
            ...
            if (isIgnored) {
                console.log(`No upgrade of "${adapter}" desired, because version "${targetVersion}" is configured to be ignored by the user. Run "${tools.appNameLowerCase} version ${adapter} --recognize" to allow this upgrade!`);
                return;
            }
            try {
            ...
            const ioPack = (await tools.getJsonAsync(repoAdapter.meta));
            if (!ioPack) {
    ...
    async upgradeController(repoUrl, forceDowngrade, controllerRunning) {
        ...
        if (repoController.version) {
            ...
            if ((ioPack?.common && ioPack.common.version === installed.common.version) ||
