...
function MHServer(hostname, logger, config, info, ips, secret) {
  ...
  function checkAuthList(ts) {
    ...
    for (const id of Object.keys(authList)) {
      if (!authList[id]) {
        delete authList[id];
      } else if (ts - authList[id].ts > 31e3) {
        delete authList[id];
      }
    }
  }
  async function sha(secret2, salt, callback) {
          ...
          return sha(secret, authList[id].salt, (shaText) => {
            if (shaText !== msg.password) {
              ...
              send({
                ...
                info,
                hostname,
                slave: !import_js_controller_common_db.tools.isLocalObjectsDbServer(config.objects.type, config.objects.host),
                result: "ok"
              }, rinfo);
            }
          });
  ...
  this.init = function() {
    ...
    server = import_node_dgram.default.createSocket({ type: "udp4", reuseAddr: true });
    server.on("error", (err) => {
    ...
    server.on("message", (msg, rinfo) => {
      ...
      for (const ids in buffer) {
        if (!lastFrame[ids]) {
          delete buffer[ids];
        } else if (now - lastFrame[ids] > 1e3) {
          delete buffer[ids];
          delete lastFrame[ids];
        }
      }
      if (lastFrame[id] && now - lastFrame[id] > 1e3) {
        buffer[id] = "";
      }
      lastFrame[id] = now;
      if (!buffer[id] && text[0] !== "{") {
